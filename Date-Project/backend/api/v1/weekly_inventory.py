...
def files(page: int = Query(1, ge=1), limit: int = Query(20, ge=1, le=100)):
    return repo.list_files(page, limit)
...
def _run(request_id):
    ...
    try:
        return run_scheduler_task(TASK_CODE, request_id=request_id, trigger_type="manual")
    except Exception:
...
def run():
    ...
    return {"request_id": request_id, "message": "已提交后台任务；结果见文件列表和任务日志"}
