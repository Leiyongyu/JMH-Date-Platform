...
class GoodcangClient:
    ...
    def fetch_storage_page(self, date_from, date_to, page):
        ...
        for attempt in range(1, self.retries + 2):
            ...
            temporary = False
            try:
                with self.http.stream("POST", self.url, json=body) as response:
            ...
            if not temporary or attempt > self.retries:
                raise GoodcangRequestError(
                    f"谷仓仓租接口失败；接口={PATH} page={page} "
                    f"attempt={attempt}/{self.retries + 1} reason={reason}"
                ) from None
            LOG.warning("谷仓仓租有限重试 path=%s page=%s attempt=%s/%s reason=%s delay=%ss",
                        PATH, page, attempt, self.retries + 1, reason, delay)
            time.sleep(delay)
