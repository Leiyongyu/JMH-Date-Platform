...
from modules.tax_refund.repository import get_all_exports
from modules.tax_refund.repository import list_receivables
from modules.tax_refund.repository import get_all_inventory
from modules.tax_refund.schemas import ErrorResponse, RefundPackageTaskRequest, TaskAcceptedResponse
...
@router.post(
    ...
    openapi_extra={
        'requestBody': {
            ...
            'content': {
                'application/json': {
                    'schema': RefundPackageTaskRequest.model_json_schema(),
                },
                'multipart/form-data': {
                    'schema': {
                        'type': 'object',
...
async def create_api_task(request: Request):
    ...
    created_by = str(
        request.headers.get('X-ERP-User') or payload.pop('created_by', None) or 'ERP'
    ).strip()[:64]
    ...
    elif task_type == TASK_REFUND_PACKAGE:
        try:
            validated = RefundPackageTaskRequest.model_validate({
                'task_type': task_type,
                **payload,
            })
