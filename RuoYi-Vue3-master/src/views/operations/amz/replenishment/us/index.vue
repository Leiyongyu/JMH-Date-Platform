...
<script setup name="AmzUsReplenishment">
import { ref, reactive, toRefs, h, resolveComponent, getCurrentInstance, computed, watch } from 'vue'
import { searchUsReplenishment, refreshUsReplenishment } from '@/api/operations/amz/usReplenishment'
...
async function loadBreakdown(row, field) {
  ...
  try {
    const res = await request({
      url: '/operations/amz/replenishment/us/sales-breakdown/search',
      method: 'post',
...
function loadStoreOptions() {
  request({ url: '/operations/amz/replenishment/us/store-names', method: 'get' }).then(res => {
    storeOptions.value = res.data || []
  })
}
function loadWarehouseOptions() {
  request({ url: '/operations/amz/replenishment/us/warehouse-names', method: 'get' }).then(res => {
...
async function onAmzCellBlur(row, field) {
  ...
  try {
    if (field === 'cat') {
      await request({ url: '/operations/amz/replenishment/us/override', method: 'post', data: { sid: String(row.sid||''), sellerSku: row.sellerSku, productCategory: v || '' } })
    } else if (field === 'rem') {
      await request({ url: '/operations/amz/replenishment/us/override', method: 'post', data: { sid: String(row.sid||''), sellerSku: row.sellerSku, remark: v || '' } })
    } else {
      await request({
        url: '/operations/amz/replenishment/us/update-qty-receive',
        method: 'post',
        data: { warehouseSku: row.warehouseSku, warehouseName: row.warehouseName, value: v || null }
      })
    }
...
const columnDefs = [
  ...
  { key: 'profitRate90d', label: '90天利润率', align: 'right', width: 120, sortable: true, format: 'percentNumber', filterType: 'number' }
]
const {
  showColumnDrawer,
  columnConfigLoaded,
  ...
  initColumnConfig,
  applyColumnConfig
} = useColumnConfig('operations:amz:replenishment:us', columnDefs, fixedColumnKeys, requiredColumnKeys)
const data = reactive({
  queryParams: {
    pageNum: 1,
    pageSize: 50,
...
function getList() {
  ...
  searchUsReplenishment(body).then(response => {
...
async function handleRefresh() {
  ...
  loading.value = true
  try { await refreshUsReplenishment(); await getList() }
  finally { loading.value = false }
}
...
async function handleSyncAll() {
  ...
  try {
    ...
    proxy.$modal.msgSuccess('AMZ同步任务已提交，正在后台执行…')
    startPolling(sid, (result) => {
      syncing.value = false
      getList()
      if (result.status === 'SUCCESS') {
        proxy.$modal.msgSuccess(`AMZ同步完成: ${result.successSteps || '?'}/${result.totalSteps || '?'} 步成功`)
      } else if (result.status === 'PARTIAL') {
        proxy.$modal.msgWarning(`AMZ同步部分完成: ${result.successSteps || '?'}/${result.totalSteps || '?'} 步成功, 查看同步日志了解详情`)
      } else {
        proxy.$modal.msgError('AMZ同步失败: ' + (result.errorMessage || '未知错误'))
      }
    })
  } catch (e) {
...
function handleExport() {
  ...
  request({ url: '/operations/amz/replenishment/us/export', method: 'post', data: body, responseType: 'blob' }).then(res => {
    const blob = new Blob([res]); const a = document.createElement('a'); a.href = URL.createObjectURL(blob)
    a.download = `amz_us_replenishment_${new Date().getTime()}.xlsx`; a.click()
  })
}
