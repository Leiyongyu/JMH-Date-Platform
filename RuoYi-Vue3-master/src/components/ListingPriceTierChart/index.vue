<template>
  <section v-loading="loading" class="price-panel" :aria-labelledby="`price-tier-${platform}`">
    <header>
      ...
      <el-button size="small" icon="Refresh" circle :disabled="loading" title="按当月汇率重新计算本地数据，不拉取接口" aria-label="重新统计价格分层" @click="load(true)" />
    </header>
    ...
    <template v-else-if="report?.state === 'READY'">
      ...
      <div v-if="report.missing_currencies.length" class="warning">缺当月汇率：{{ report.missing_currencies.join('、') }}，相关记录未归档。</div>
...
const ruleDescription = computed(() => `${props.platform === 'amz'
  ...
  : 'eBay来源：GetMyeBaySelling的ods_ebay_store_listing_latest.current_price；多规格按变体SKU及价格，不重复计父商品。'}按当月dim_lingxing_currency_month.${presentation.value.rateField}换算${currencyLabel.value}${props.platform === 'ebay' ? '（美元原价不换算）' : ''}，不截断汇率，分档前不舍入。店铺+站点内完整SKU取最低有效${currencyLabel.value}价格、只计一次，跨站点相加。0为有效价格；缺SKU、价格或汇率异常单独提示、不计入占比；不按中间码合并、不排除PC。刷新只重新计算本地统计仓库，不拉接口。`)
const rateDescription = computed(() => `汇率月份 ${report.value?.rate_month || '--'}；${presentation.value.formula}。使用的${presentation.value.rateField}：${Object.entries(report.value?.rates || {}).map(([k, v]) => `${k} ${v ?? '--'}`).join('、')}。仅使用计算当月汇率，不回退旧月份。`)
...
const anomalyText = n => [n.unclassified_sku_count ? `${n.unclassified_sku_count}个SKU未归档` : '', n.missing_sku_rows ? `${n.missing_sku_rows}条缺SKU` : '', n.invalid_price_rows ? `${n.invalid_price_rows}条价格/站点异常` : '', n.missing_rate_rows ? `${n.missing_rate_rows}条缺当月汇率` : '', n.missing_shop_rows ? `${n.missing_shop_rows}条店铺未匹配` : ''].filter(Boolean).join('；')
...
async function load(refresh = false) {
  ...
  try {
    ...
    if (!disposed) error.value = '报表加载或统计失败，请检查权限、统计表和服务日志。原始数据未修改。'
  } finally {
