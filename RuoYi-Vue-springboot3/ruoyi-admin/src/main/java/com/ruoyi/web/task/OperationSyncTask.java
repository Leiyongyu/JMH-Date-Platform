...
/** 运营数据同步定时任务。定时任务和手动按钮共用同一套服务+同一把锁。 */
...
public class OperationSyncTask
{
    ...
    // ==================== 低频基础同步 ====================
    public void syncGoodcangWarehouse() { exec("goodcang_warehouse", "谷仓-仓库信息", "/base_data/get_warehouse", null,
            () -> SpringUtils.getBean(GoodcangWarehouseSyncService.class).syncWarehouses()); }
    public void syncLingxingWarehouse() { exec("warehouse", "领星-仓库信息", "erp/sc/data/local_inventory/warehouse", null,
            () -> SpringUtils.getBean(LingxingWarehouseSyncService.class).syncWarehouses()); }
    public void syncLingxingShop() { exec("shop_list", "领星-店铺列表", "pb/mp/shop/v2/getSellerList", null,
    ...
    public void syncGoodcangProduct() { exec("goodcang_product", "谷仓-商品信息", "/product/get_product_sku_list", null,
    ...
    public void syncLingxingPurchaseOrder() { exec("purchase_order", "领星-采购单", "erp/sc/routing/data/local_inventory/purchaseOrderList", null,
            () -> SpringUtils.getBean(LingxingPurchaseOrderSyncService.class).sync()); }
    public void syncLingxingPurchasePlan() { exec("purchase_plan", "领星-采购计划", "erp/sc/routing/data/local_inventory/getPurchasePlans", null,
    ...
    private void exec(String type, String name, String api, String lockKey, SyncRunner runner) {
        if (lockKey != null) {
            ...
            if (!redis.tryLock(lockKey, 600)) {
                LOG.info("[SKIP] {} — {} 被占用，跳过本次调度", name, lockKey);
                return;
            }
            try { executeWithLog(type, name, api, runner); }
            finally { redis.unlock(lockKey); }
        } else {
