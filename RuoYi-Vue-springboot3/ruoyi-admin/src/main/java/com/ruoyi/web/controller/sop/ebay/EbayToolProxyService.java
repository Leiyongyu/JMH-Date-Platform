...
/** 将ERP同源请求安全、流式地转发到Python eBay价格工具子应用。 */
@Service
public class EbayToolProxyService
{
    ...
    private final EbayToolPythonProperties properties;
    private final HttpClient httpClient;

    public EbayToolProxyService(EbayToolPythonProperties properties)
    {
            ...
            ImageSopSessionService.SessionContext session) throws IOException
    {
        ...
        {
            writeJsonError(response, 405, "eBay工具代理仅允许GET、POST和HEAD请求");
            return;
        }
        ...
        catch (InterruptedException e)
        {
            ...
            writeJsonError(response, 502, "eBay价格工具服务调用被中断");
        }
        ...
        {
            writeJsonError(response, 502, "eBay价格工具Python服务不可用: " + e.getMessage());
        }
    }
            ...
            HttpRequest.Builder builder)
    {
        ...
        {
            String boundary = "----JmhEbayTool" + UUID.randomUUID().toString().replace("-", "");
    ...
    private InputStream multipartInputStream(MultipartHttpServletRequest request, String boundary)
    {
        ...
        {
            throw new IllegalStateException("构造eBay工具上传请求失败", e);
        }
    }
