...
CREATE TABLE `amz_replenishment_formula_config`  (
  ...
  `deduct_fba_inbound_working` tinyint(1) NOT NULL DEFAULT 1 COMMENT '扣减FBA计划入库',
  ...
  `deduct_pending_ship_qty` tinyint(1) NOT NULL DEFAULT 0 COMMENT '历史字段：待出库不再参与补货计算',
  ...
  `formula_replenish` varchar(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL DEFAULT '({d14}*{w14} + {d30}*{w30} + {d60}*{w60}) * {replenish} - {purchases} - {domestic} - {fba} - {inboundWorking}' COMMENT '补货量公式',
  `formula_restock` varchar(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL DEFAULT '({fba} - (({d14}*{w14} + {d30}*{w30} + {d60}*{w60}) * {replenish} - {purchases} - {domestic} - {fba} - {inboundWorking})) / ({d14}*{w14} + {d30}*{w30} + {d60}*{w60})' COMMENT '补货时间公式',
