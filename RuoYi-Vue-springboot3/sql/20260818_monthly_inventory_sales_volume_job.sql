...
-- 每月11日23:00拉取上个完整自然月，填充月度库存报表实际达成及销量。
...
SET job_name = '月度库存实际达成及销量填充',
    ...
    update_by = 'SYSTEM',
    update_time = NOW(),
    remark = '每月11日23:00一次拉取上个完整自然月Amazon订单利润amount和volume并按月覆盖Python ODS、重建实际达成及销量DWD；eBay销量由jmh_data_platform.ebay_sales按payment_time汇总quantity。'
...
SELECT
    ...
    '2', '1', '0',
    'SYSTEM', NOW(),
    '每月11日23:00一次拉取上个完整自然月Amazon订单利润amount和volume并按月覆盖Python ODS、重建实际达成及销量DWD；eBay销量由jmh_data_platform.ebay_sales按payment_time汇总quantity。'
WHERE NOT EXISTS (
    SELECT 1
    FROM sys_job
    WHERE invoke_target IN (
